...
@implementation NSObject (XZReflection)
...
{
    return [[self class] xz_propertyKeys];
}
+ (NSArray *)xz_propertyKeys {
...
{
    return [[self class] xz_propertiesInfo];
}
/**
 *  属性列表与属性的各种信息
 */
+ (NSArray *)xz_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self xz_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)xz_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] xz_propertiesInfo];
...
//            NSString *argString = [NSString stringWithCString:arg encoding:NSUTF8StringEncoding];
            [arguments addObject:[[self class] xz_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] xz_decodeType:returnType];
        NSString *encodeString = [[self class] xz_decodeType:encoding];
...
-(NSDictionary *)xz_protocolList{
    return [[self class]xz_protocolList];
}
+ (NSDictionary *)xz_protocolList
{
...
+ (NSArray *)xz_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] xz_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)xz_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)xz_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString xz_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}

...
/**
 *  `XZEventHandler` is the type of event handler block
 *
 *  @param event Object for the event which triggers current handler block.
 */
typedef void (^XZEventHandler)(XZEvent *event);

/**
 `XZEvent` is the event object passed into handler block.
 */
@interface XZEvent : NSObject
...
/**
 *  `XZPubSub` category on `NSObject`, so you can call methods in `XZPubSub` on any subclass of `NSObject`
 */
@interface NSObject (XZPubSub)

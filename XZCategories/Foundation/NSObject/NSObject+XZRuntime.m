...
BOOL xz_method_swizzle(Class klass, SEL origSel, SEL altSel)
{
    if (!klass)
        return NO;
...
void xz_method_append(Class toClass, Class fromClass, SEL selector)
{
...
void xz_method_replace(Class toClass, Class fromClass, SEL selector)
{
    if (!toClass || !fromClass || ! selector)
        return;
...
{
    xz_method_swizzle(self.class, originalMethod, newMethod);
}

+ (void)xz_appendMethod:(SEL)newMethod fromClass:(Class)klass
{
    xz_method_append(self.class, klass, newMethod);
}

+ (void)xz_replaceMethod:(SEL)method fromClass:(Class)klass
{
    xz_method_replace(self.class, klass, method);
}

- (BOOL)xz_respondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.class xz_instancesRespondToSelector:selector untilClass:stopClass];
}
...
{
    return [self.superclass xz_instancesRespondToSelector:selector untilClass:stopClass];
}

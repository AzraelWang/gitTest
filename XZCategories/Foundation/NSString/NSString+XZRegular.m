...
@implementation NSString (XZRegular)
...
- (BOOL)xz_isValidChinese;
{
    ...
    return [self xz_isValidateByRegex:chineseRegex];
}
...
{
    BOOL lengthValid = [self xz_isValidWithMinLenth:minLenth maxLenth:maxLenth];
    if (lengthValid){
...
- (BOOL)xz_passwordIsSimple
{
    ...
    if (![self xz_isCharAndNum]) return NO;
    
    NSString *firstObj = [self substringWithRange:NSMakeRange(0, 1)];
    BOOL same = YES;
    BOOL ascending = YES;
    BOOL descending = YES;
    
    for (int i = 1; i < self.length; i++) {
        NSString *subStr1 = [self substringWithRange:NSMakeRange(i-1, 1)];
        NSString *subStr2 = [self substringWithRange:NSMakeRange(i, 1)];
        int ascNum1 = [subStr1 characterAtIndex:0];
        int ascNum2 = [subStr2 characterAtIndex:0];
        
        same = same && [firstObj isEqualToString:subStr2];
        ascending = ascending && ascNum1 + 1 == ascNum2;
        descending = descending && ascNum1 - 1 == ascNum2;
    }
...
{
    NSString *mobileRegex = @"^(0|86|17951)?(13[0-9]|15[012356789]|17[0678]|18[0-9]|14[457])[0-9]{8}$";
    return [self xz_isValidateByRegex:mobileRegex];
}
                      ...
                      maxLenth:(NSInteger)maxLenth
{
    ...
    return [self xz_isValidateByRegex:lengthRegex];
}
